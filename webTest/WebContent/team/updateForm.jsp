		...
		<table>
			...
			<tr>
				...
				<td>	<label><input type="checkbox" name="skill"
						value="Java"
						<%if(dto.getSkills().indexOf("Java")!=-1) out.print("checked"); %>
						>java</label> 
						<label><input type="checkbox"
						name="skill" value="JSP"
						<%if(dto.getSkills().indexOf("JSP")!=-1) out.print("checked"); %>
						>JSP</label> 
						<label><input
						type="checkbox" name="skill" value="MVC"
						<%if(dto.getSkills().indexOf("MVC")!=-1) out.print("checked"); %>
						>MVC</label> 
						<label><input
						type="checkbox" name="skill" value="Spring"
						<%if(dto.getSkills().indexOf("Spring")!=-1) out.print("checked"); %>
						>Spring</label> 
						<label><input
						type="checkbox" name="skill" value="Ajax"
						<%if(dto.getSkills().indexOf("Ajax")!=-1) out.print("checked"); %>
						>Ajax</label></td>
			</tr>
			<tr>
				<th>취미</th>
			...
			<tr>
				...
				<td><input type="text" name="address" size="45" type="text"
					id="sample6_address" value="<%=dto.getAddress() %>"> <input type="text"
					name="address2" size="45" type="text" id="sample6_address2"
					value="<%=dto.getAddress2() %>"></td>
			</tr>
		</table>
		<div id="h">
			<button type="button" onclick="incheck(this.form)">수정</button>
			<button type="reset">입력취소</button>
			<button type="button" onclick="location.href='./list.jsp'">뒤로가기</button>
		</div>
	</form>
